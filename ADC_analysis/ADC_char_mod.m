...
casenum=1; % select case number you want to plot (case 1: Vref=Vminus, case 2 : Vref=Vminus +VdV) 
channel=1;%Select channel
dp=6;
addpath('../');
R={
'MasdaX	2010-08-18	measurements	PSI-1_DACCHAR	20100818T163801_DACCHAR'
...
%           chsel=sort(MATSET.setup.Dpottoch);


[Voltsperch_cards,adccards] = meas_adctovolts_dac([r2{j}(1:end-3) 'fmd'], [20,40]);
figure(1);
plot(datadVADC_mean{k}(j1,i),Voltsperch_cards{dp},'r*');
title([titleArray sprintf('\n Digipot %d',dp);],'fontsize',15,'fontweight','bold');
xlabel('Measured V','fontsize',15,'fontweight','bold');
ylabel('Calculated V','fontsize',15,'fontweight','bold');

hold on;
figure(2);
plot(datadVADC_mean{k}(j1,i),Voltsperch_cards{3}-datadVADC_mean{k}(j1,i),'r*');
hold on;
...
%    for k1=startn:finishp
k1=1;
                ind{k1}=find(datadVADC_mean{k1}>-14 & datadVADC_mean{k1}<14);
                ind1{k1}=find(datadVADC_mean{k1}>-17 &datadVADC_mean{k1}<17);
        in2=in2+1;
       % uVperADCmatrix(1,in2)=Vrefmat{k1};
...
% %             Channel_det{ch}.Vref{k1}.VperADC=uVperADCmatrix(ch+1,k1+1)*1e-6;
% 
%             Channel_det{ch}.xval_dV=datadVADC_mean{k1};
%             Channel_det{ch}.yval_ADC=dataMeanFilterch{k1}(:,1,ch);
%             %Channel_det{ch}.Vref{k1}.Vrefval=Vrefmat{k1};
%             Channel_det{ch}.VperADC=uVperADCmatrix(ch+1,k1+1)*1e-6;
%             
% %             
%  %    end
