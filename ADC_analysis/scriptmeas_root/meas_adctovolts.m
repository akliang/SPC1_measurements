function [Voltsperch_cards,adccards] = meas_adctovolts(A, frames)
...
MEASDIR=['../../' date '/measurements/PSI-1_ADCCHAR/' timestamp '_ADCCHAR'];
fmdfile=[MEASDIR '/' A];
...
' meas_adcchar.m first and then try again'];
...
if(isempty(matfile))
    for k=1:numofadccards
        searchmat{k}=dir([ADCDIR '*'  '_' g3 '_' adccards{k} '*.mat']); 
