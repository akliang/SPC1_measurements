...
if(strcmp(setup.ADCtype,'+-20V'))
    ...
    DAC=dactest(setup.Vtoset,setup.PF_analogCard);
...
[Voltsperch_cards,adccards]=meas_adctovolts_dac([meas.BaseName(split(end)+1:end) '.fmd'],[20,40]);
...
channel=adcnoise(meas.AcqFile);
    ...
    for ind=1:size(Voltsperch_cards,1)
        ...
        for dpot=1:8
            ...
            display(sprintf('D%02d- V1: %6.3f (std: %4.2f ADC) V2: %6.3f (std: %4.2f ADC) ADC diff : %6.1f I:%6.3f mA R: %6.1f ohms',dpot,Voltsperch_cards{ind,chp(1)},channel.std(chp(1)),Voltsperch_cards{ind,chp(2)},channel.std(chp(2)),(channel.ADC(chp(1))-channel.ADC(chp(2))),((Voltsperch_cards{ind,chp(1)}-Voltsperch_cards{ind,chp(2)})/24)*1000,R)); 
        end
    end
end
