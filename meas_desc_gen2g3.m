...
env.V(end+1)=  0.0  ; id.Voff    =numel(env.V);  id.Vglclamp=numel(env.V);  % Gate Line LOW                                   % Vout2
env.V(end+1)=  1.0  ; id.VQinj   =numel(env.V);                             % for gain test, toggle between 1 and 2 V         % Vout3
env.V(end+1)=  3.7  ; id.Vref    =numel(env.V);  id.Vdlgnd  =numel(env.V);  % Vref=0.856 for PSI-1; 2.303  for PSI2/3   % Vout4, or generated by R/R on Masda-R card
env.V(end+1)=  0.0  ; id.Vbias   =numel(env.V);                             % Vbias=-3.0 for PSI-1; Vrst/Vgnd - 3V for PSI2/3 % Vout5
...
env.V(end+1)=  7.0  ; id.VDatLHI =numel(env.V);  id.VdlreadHI =numel(env.V);% Data Logic HIGH , e.g.  15V                     % Vout11
