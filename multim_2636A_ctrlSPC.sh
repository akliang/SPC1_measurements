...
function init_comp() {
  send_cmd "v1(4) v2(-4) v5(4) v6(-4)"
}

function do_compsweep() {  # sweep through bias and thresh
  init_comp

  VBCH="v3"
  VTCH="v4"
  VBIAS=$( octave --quiet --eval "for v=[-4:0.25:4]; fprintf('%.3f ',v); end" )
  VTHRESH=$( octave --quiet --eval "for v=[-4:0.25:4]; fprintf('%.3f ',v); end" )
  for VB in $VBIAS; do
    SWEEP="compSweep_VB=$VB"
    CH=$VTCH
    [ "$TO" == "" ] && TO=2
    VALS="$VTHRESH"
    send_cmd "$VBCH($VB)"
...
function do_compsweep_thresh() {  # sweep through thresh only)
  init_comp
...
function test_res() {  # step through voltages to test resistor
  send_cmd "v1(0) v2(0) v3(0) v4(0)"
  CH="v3"
  for F in 2 4 6 8 -2 -4 -6 -8; do
    read -p "Press any key to change $CH to $F..." -n1 -s
    send_cmd "$CH($F)"
  done
}


function amp_bias_sweep() {  # step through voltages to test resistor
