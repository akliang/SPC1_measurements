...
setup.ARRAYTYPE='M88';
setup.WAFERCODE='7293-11B';
setup.PLATFORM='V3';
setup.PF_dataCards='x1-x2-x3-x4'; % first is the outermost, last is the innermost
...
setup.PF_dataBoardDIPs='01100000'; % [ PG3 PG4 PG5 CTRL9 CTRL10 CTRL12 CTRL11 ?? ]
%setup.PF_dataBoardJumper='JP1=Vbias1';
setup.PF_gateCards='y1-y2-y3-y4';
setup.PF_analogCard='V1Nx';
...
env.V=[];
env.V(end+1)= -8.0  ; id.AVoff   =numel(env.V);                             % Test Point near to Gate Card
env.V(end+1)=  10.0 ; id.Von     =numel(env.V);                             % Test Point near to Gate Card
...
env.V(end+1)=  3.0  ; id.VQinj   =numel(env.V);                              % Vn.Qinj=2.0; % toggle between 1 and 2 V         % Vout3
...
meas.MeasCond='Signal'; multi.R22=0; % setting: PG4
...
meas.DirName=[ '/home/user/Desktop/Hao/MD88_7293_11B/2011-11-22/Trapping_16_2V_100mA_10ms_72kVp_49472R2/script'];
meas.append='GOC-Dark';
...
meas.BaseName=[ meas.DirName ...
    meas.MeasID '-' meas.append ... '_' meas.DUT   ...     
...
meas.AcqFile=[ meas.BaseName '.bin' ];
...
meas.R=[
 %value PSI-1 PSI-2 PSI-3   %name  bits  (default)units        Description
        ...
        0                   %R23         N/I
        0                   %R24         N/I
        240                 %R25   12    flashes (F12==1)   # of LED flashes per acquisition cycle (0: really no LED flash)
        multi.R26           %R26   16    cycles             # of Ignore Cycles
        multi.R27           %R27   16    cycles             # of  Data  Cycles (continous if R26==0&&R27==0)
        20                  %R28   12    cycles  (F12==1)   Initial Cycle to start LED flashes, zero-indexed (0: start at first cycle)
        50                  %R29   12    cycles  (F12==1)   # of Data Cycles with LED flashes (R29==0&R28==0: always flashing! R29==0&&R28>0: really no cycle with LED flashes, but still Tau_8,Tau_9&Tau10 Timing during Tau_2?)
