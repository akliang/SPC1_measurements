...
env.V(end+1)=  1.0  ; id.Vbias   =numel(env.V);                   % Vn.Vbias=-3.0;   env.V(end+1)=env.V(id.Vgnd)   env.V(id.Vreset)  % Vout4
env.V(end+1)=  1.0  ; id.VQinj   =numel(env.V);                              % Vn.Qinj=2.0; % toggle between 1 and 2 V         % Vout3
env.V(end+1)=  3.0 ; id.Vref    =numel(env.V);                             % Vn.Vref=0.856; 2.303  for PSI2/3 cards  %   usually generated by R/R on Masda-R card
...
env.I.V24m=0.086;  % Current in amperes on the BK PRECISION -24V power supply
env.I.V24p=0.097;  % Current in amperes on the BK PRECISION +24V power supply
