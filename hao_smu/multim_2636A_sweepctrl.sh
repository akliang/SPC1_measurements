...
# modified 2011-07-14 by martin for hao's and qihua's HgI measurements
...
function biason() {
  SWEEP="bias"
  VALS=$( octave --quiet --eval "for v=1.0:-0.2:-6.8; disp(v); end" )
  TO=0.2
  CH="v1"
  do_sweep true
}
...
function biasoff() {
  SWEEP="bias"
  VALS=$( octave --quiet --eval "for v=-6.8:0.2:1.0; disp(v); end" )
  TO=0.2
  CH="v1"
  do_sweep true
}
...
function biasinit() {
  SWEEP="bias"
  VALS=$( octave --quiet --eval "for v=0:-0.4:-6.8; disp(v); end" )
  TO=1
  CH="v1"
  do_sweep true
}
