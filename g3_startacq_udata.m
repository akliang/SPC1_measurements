function message = g3_startacq_udata( host, port, R, udata, AcqFile, SORTMODE, nuke, wait4end )
    ...
    while true
        ...
        %try
            ...
            % set user payload (string version, ending up in AMD file)
            PAYHEAD=sprintf('num2str of userdata:\n');
            PAYLOAD=num2str(udata');
            send_uint( [ 1002 numel(PAYHEAD)+numel(PAYLOAD) ], 32 );
            send_uint( PAYHEAD, 8 );
            send_uint( PAYLOAD, 8 );
            read_response();

            % enable or disable flags/parameters
            % enable/disable send_eof (end-of-frame)
            send_uint( [ 1004 12 ], 32 );
            send_uint( [1 0 0], 32 );
            read_response();
            ...
            % set userdata to save to FMD
            % type: 1004
            % length: 8+payload length
            % 1004 9 offset data...
            send_uint( [ 1004 12+numel(udata)*2 ], 32 );
            send_uint( [ 9 0 0 ], 32 ); % 9: fmd user data, 0 0: offset address
            send_int(  udata, 16 );
            read_response();
   ...
   %end
    ...
    function send_int( valxx, bitwidth )
        for snrxx=1:numel(valxx);
            xx=double(valxx(snrxx));
...
% set data sorting mode (SORTMODE)
% type: 1003
% length: 4
% payload: int
%  0: NO DATA SORTING
...
% set JJAM parameters/set value command:
% type: 1004
% length:  8 (id/value pair) or variable (outdated!)
% length: 12 (id int, value long) as of 2010-02-08 19:10
%  id/name(value):
...
%  value: 0 off, 1 on
%  NEW as of 2010-01-28:
%     8 frame_metadata_version (overrides JJAM internal frame_metadata)
%     9 frame_metadata (sets frame metadata, variable length!)
%    10 frame_metadata_minimum_user_payload_offset (default: 200)
%       (self-clearing - needs testing!)
%  (later change value to long)

% to set .fmd user data:
% type: 1004
% length: 8+payload length
% 1004 9 offset data...
...
% 2101: acqusition metadata
% 2102: frame metadata
% 2103: image
% 2104: eof (end-of-frame)
% 2105: eoc

% 2001: general ack
% 2002: nack (never sent as of 2010-02-08)
% 2003: queue stopped (e.g. when user presses stop on gui)
...
%{
 Acquisition Header for automated acquisition as of 2010-01-18 (.amd)
 5000 byte total
